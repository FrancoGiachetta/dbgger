...
{
    auto format = [](auto t) {
        // decltype is used to get the type of t.
        ...
        {
            // format an hexadecimal with a padding of sizeof(t) * 2 + 2 zeros.
            return fmt::format("{:#0{}x}", t, sizeof(t) * 2 + 2);
        }
        else
        {
            // format an hexadecimal with a padding of 4 zeros.
            return fmt::format("[{:#04x}]", fmt::join(t, ","));
        }
    };

    // Print all registers of just GPRs.
    if (args.size() == 2 or (args.size() == 3 and args[2] == "all"))
    {
        for (auto &info : sdb::g_register_infos)
        {
            // We don't print orig_rax since it's not a GPR.
