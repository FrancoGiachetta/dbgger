...
template <class T> sdb::byte128 widen(const sdb::register_info &info, T t)
{
    ...
    else if constexpr (std::is_signed_v<T>)
    {
        if (info.format == register_format::uint)
        {
            ...
            {
            case 1:
                return to_byte128(static_cast<std::uint8_t>(t));
            case 2:
                return to_byte128(static_cast<std::uint16_t>(t));
            case 4:
                return to_byte128(static_cast<std::uint32_t>(t));
            case 8:
                return to_byte128(static_cast<std::uint64_t>(t));
            }
        }
    }
    ...
    std::visit(
        [&](auto &v) {
            if (sizeof(v) <= info.size)
            {
                ...
                std::copy(val_bytes, val_bytes + sizeof(val_bytes + info.size), bytes + info.offset);
            }
            else
            {
                std::cerr << "sdb::registers::write called with "
